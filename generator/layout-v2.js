(() => {
  ...
  function bindPreview(urlInput,fileInput,preview){
    const draw=()=>{const src=urlInput?.value.trim();preview.innerHTML=src?`<img src="${esc(src)}" alt="Портрет">`:'';};
    urlInput?.addEventListener('input',draw);fileInput?.addEventListener('change',()=>setTimeout(draw,40));draw();
  }
  ...
  function buildSingle(){
    ...
    source.dataset.v2='1';
    const legacy=preserveLegacyInputs(source,'s');
    eqCard.appendChild(source);source.classList.add('layout-v2-single-visual');
    source.innerHTML=`
      ...
      <div class="layout-v2-portrait"><div class="field" style="margin:0"><label>Главное фото персонажа</label><input id="v2-s-portrait-url" type="url" placeholder="Ссылка на фото"><button type="button" class="tab layout-v2-upload" id="v2-s-portrait-file">Загрузить фото</button></div><div id="v2-s-portrait-preview" class="layout-v2-preview"></div></div>
      <div class="layout-v2-aesthetic"><div class="layout-v2-aesthetic-title">ЭСТЕТИКА</div><div class="layout-v2-grid">
    ...
    $('v2-s-portrait-file').addEventListener('click',()=>portraitFile?.click());bindPreview(portraitUrl,portraitFile,preview);
    for(let i=0;i<6;i++){
      const url=$(`v2-s-img-url-${i}`),hidden=$(`s-img-url-${i}`),button=document.querySelector(`[data-file="s-img-file-${i}"]`),file=$(`s-img-file-${i}`);
      url.addEventListener('input',()=>mirrorUrl(hidden.id,url.value));button?.addEventListener('click',()=>file?.click());
    }
  }

  function buildPair(){
    const host=$('pair-visual');if(!host||host.dataset.v2==='1')return;host.dataset.v2='1';
    const legacy1=preserveLegacyInputs(host,'p1'),legacy2=preserveLegacyInputs(host,'p2');
    host.innerHTML=`
    ...
    host.appendChild(legacy1);host.appendChild(legacy2);
    ...
    for(const prefix of ['p1','p2']){
      ...
      url.addEventListener('input',()=>mirrorUrl(hidden.id,url.value));$(`v2-${prefix}-portrait-file`).addEventListener('click',()=>file?.click());bindPreview(url,file,preview);
    }
    for(let i=0;i<6;i++){const url=$(`v2-pair-img-${i}`),hidden=$(`p1-img-url-${i}`);url.addEventListener('input',()=>mirrorUrl(hidden.id,url.value));}
  }

  function start(){
    injectStyles();const observer=new MutationObserver(()=>{buildSingle();buildPair();});observer.observe(document.body,{childList:true,subtree:true});buildSingle();buildPair();
  }
