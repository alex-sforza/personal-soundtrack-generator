(() => {
  ...
  function stashLegacy(prefix){
    const holder=document.createElement('div');holder.className='v3-legacy';holder.setAttribute('aria-hidden','true');
    const ids=[`${prefix}-portrait-url`,`${prefix}-portrait-file`];
    for(let i=0;i<6;i++)ids.push(`${prefix}-img-url-${i}`,`${prefix}-img-file-${i}`);
    ids.forEach(id=>{const el=$(id);if(el){el.classList.add('v3-legacy');if(el.parentElement)el.parentElement.classList.add('v3-legacy');holder.appendChild(el);}});
    return holder;
  }
  ...
  function buildSingle(){
    ...
    if(!host||!eq||host.querySelector('.v3-title')){
      if(!host||!eq)return;
      const legacy=stashLegacy('s');
  ...
  function buildPair(){
    const host=$('pair-visual');if(!host||host.querySelector('.v3-pair-head'))return;
    const legacy1=stashLegacy('p1'),legacy2=stashLegacy('p2');
    host.innerHTML=`
  ...
  function start(){styles();const run=()=>{buildSingle();buildPair();};new MutationObserver(run).observe(document.body,{childList:true,subtree:true});run();}
